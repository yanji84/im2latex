...
class RecurrentNet(object):
    def __init__(self, variable_scope, x, y, image, mask):
        ...
        self.attended_feature_size = constants['attended_feature_size']
        ...
        with tf.variable_scope(variable_scope):
            ...
            self.Wfc = tf.Variable(tf.random_normal([self.attended_feature_size, self.n_classes]))
            self.bfc = tf.Variable(tf.zeros(self.n_classes))
            self.intial_context_w = tf.Variable(tf.random_normal([self.attended_feature_size, self.hidden_dim]))
            self.intial_context_b = tf.Variable(tf.zeros(self.hidden_dim))

    def lstm_step_forward(self, x_idx, prev_h, prev_c):
        Wx = self.Wx
        Wh = self.Wh
        Wa = self.Wa
        b = self.b
        ...
        # take out word embedding for google house number problem
        #activation_vector = tf.add(tf.matmul(x, Wx), tf.matmul(prev_h, Wh))
        activation_vector = tf.add(tf.matmul(prev_h, Wh) + self.b)
        ...
        attended_features = self.convNet.extract_conv_features(attended)
        attended_features = tf.reshape(attended_features, [-1, self.attended_feature_size])

        return next_h, next_c, attended_features

    def lstm_forward(self):
        generated = []
        generated_logits = []
        features = self.ctx_convnet.extract_conv_features(self.image)
        features = tf.reshape(features, [-1, self.attended_feature_size])
        ...
        for t in range(self.n_steps):
            generated.append(xt)
            if t < self.n_steps - 1:
                hprev, cprev, attended_features = self.lstm_step_forward(x[t,:], hprev, cprev)
                logits = tf.matmul(attended_features, self.Wfc) + self.bfc
                generated_logits.append(logits)
                xt = tf.cast(tf.argmax(logits, 1), tf.int32)
                cost_at_t = tf.nn.softmax_cross_entropy_with_logits(logits, y[t,:,:])
                cost_at_t = tf.mul(cost_at_t, mask[t,:])
    ...
    def sample(self):
        ...
        for t in range(self.n_steps):
            generated.append(x)
            hprev, cprev, attended_features = self.lstm_step_forward(x, hprev, cprev)
            logits = tf.matmul(attended_features, self.Wfc) + self.bfc
